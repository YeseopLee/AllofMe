...
(2021.12.27 추가)\
edittext의 text값 중 일부를 viewModel의 data모델에 전달하는것이 view와 viewmodel의 관심사 분리에 적합하지 않은 방법이라는 생각이 들었다.\
view에서는 ui에 관련된 로직만을 수행해야 하는데, 이는 viewmodel에 데이터를 전달해주는 로직이기 때문이다.\
databinding을 활용하면 viewModel의 데이터에 실시간으로 반영할 수 있지 않을까 하여 적용해보려 했으나,\
애초에 로직 자체가 editText를 계속해서 생성해내고, 중간의 이미지를 삭제하는 과정에서 editText의 text 결합을 위해 index정보를 viewModel에 어떤식으로든 전달을 해야 하기 때문에,\
완전한 관심사 분리가 불가능한 구조로 보인다. (적어도 지금 내 실력으로는 그렇다...)\
프로젝트의 시작부터 끝까지 클린아키텍쳐를 적용하고자 노력했던 프로젝트였기에, 이부분을 해결하지 못한것에 아쉬움이 많이 남는다.
